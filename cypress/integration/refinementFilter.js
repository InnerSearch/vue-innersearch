...
describe('Test RLF with Searchbox' , () => {
    it('2nd non regression test for issue #4' , () => {
        ...
        cy.wait('@ES');
        cy.wait('@ES');
        cy.get('.gender_rlf > .is-component > .is-item > label').then ( $e => {
...
describe('Test Multiple RLF', () => {
    ...
    it('non regression test for issue #5', () => {
        ...
        cy.get(ctLabel).then(e => {
            ...
            cy.get('.gender_rlf > .is-component > :nth-child(2) > label').then(e => { expect(e.get(0).innerHTML).to.contains('15')});
        });
